...
def main():
    ...
    anime_stats_and_scores_raw = etl.join_data(config, all_anime, anime_votes_raw, anime_stats)
    anime_stats_and_scores_pct = etl.join_data(config, all_anime, anime_votes_pct, anime_stats)
    # Load to database
    try:
        ...
        with engine.connect() as conn:
            # Create schema
            etl.execute_sql(
                conn, [sql.CREATE_SCHEMA, sql.CREATE_DIM_DAY, "CREATE EXTENSION IF NOT EXISTS tablefunc;"]
            )
        # Insert raw data
